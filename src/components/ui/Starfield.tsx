...
export function Starfield({
  ...
  const shootingStarRef = useRef<{ x: number, y: number, length: number, speed: number, visible: boolean, opacity: number } | null>(null);
  ...
  useEffect(() => {
    ...
    const drawNebula = (ctx: CanvasRenderingContext2D) => {
      ...
      ctx.arc(canvas.width * 0.3, canvas.height * 0.4, canvas.width * 0.4, 0, Math.PI * 2);
      ctx.fill();
      ...
      ctx.arc(canvas.width * 0.7, canvas.height * 0.6, canvas.width * 0.3, 0, Math.PI * 2);
      ctx.fill();
    };
    ...
    const updateShootingStar = () => {
      ...
      if (!shootingStar.visible) {
        if (Math.random() < 0.005) { // 平均每200帧出现一次
    ...
    const drawShootingStar = (ctx: CanvasRenderingContext2D) => {
      if (!shootingStars || !shootingStarRef.current || !shootingStarRef.current.visible) return;
      ...
      gradient.addColorStop(0.3, `rgba(180, 180, 255, ${shootingStar.opacity * 0.6})`);
